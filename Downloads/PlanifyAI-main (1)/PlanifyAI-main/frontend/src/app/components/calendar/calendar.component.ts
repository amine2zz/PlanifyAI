...
import { Event } from '../../services/calendar.service';

@Component({
  selector: 'app-calendar',
  standalone: true,
  imports: [CommonModule, FormsModule, SuggestionsComponent],
  template: `
    <div class="calendar-container">
      <div class="header">
        ...
        <!-- put this near the top header so you actually see & click it -->
<div class="view-controls">
  <!-- existing buttons... -->
  <button class="today-btn" (click)="goToday()">Today</button>

  <!-- NEW: visible test button -->
  <button class="today-btn" style="margin-left:12px;background:#6c5ce7;" (click)="getSuggestions()">Get Suggestions</button>
</div>
        <div class="view-controls">
...
export class CalendarComponent implements OnInit {
  ...
  // inside CalendarComponent class
getSuggestions() {
  console.log('getSuggestions() clicked');
  // choose an event to check against existing events — prefer a selected event,
  // otherwise create a dummy event to test the endpoint.
  const sampleEvent: Event = this.events && this.events.length ? this.events[0] : {
    title: 'Test Meeting',
    description: 'Testing AI suggestions',
  ...
  // show we are calling
  console.log('Calling backend with event:', sampleEvent);
  // reuse your existing helper
  this.checkConflictsAndSuggest(sampleEvent);
}

checkConflictsAndSuggest(event: any) {
  const body = {
    event: event,
    existingEvents: this.events
  };

  console.log('POST /api/suggest-slots body:', body);

  this.http.post('http://localhost:5000/api/suggest-slots', body).subscribe({
    next: (res: any) => {
      console.log('API response:', res);
      this.suggestions = res.suggestions || [];
      console.log('Parsed suggestions:', this.suggestions);
      this.showSuggestionModal = !!(this.suggestions && this.suggestions.length);
      if (!this.showSuggestionModal) console.warn('No suggestions returned or empty array.');
    },
    error: (err) => {
...
onSuggestionSelected(suggestion: Suggestion) {
  ...
  // You can auto-fill your event creation form with this suggestion
